...
void model_test(bool use_shl) {
  std::string model_name{"resnet50-v1-7-qdq.onnx"};
  ...
  std::vector<std::vector<float>> shl_out = test_model(model_path.c_str(), input_names, input_data, input_shape, use_shl, "resnet50");

  if (use_shl) {
    for (size_t i = 0; i < shl_out.size(); i++) {
      std::string out_file_path = curt_path + "/shl_resnet50_out" + std::to_string(i) + ".bin";
      save_float_array(shl_out[i].data(), shl_out[i].size(), out_file_path.c_str());
    }
  } else {
    for (size_t i = 0; i < shl_out.size(); i++) {
      std::string out_file_path = curt_path + "/cpu_resnet50_out" + std::to_string(i) + ".bin";
      save_float_array(shl_out[i].data(), shl_out[i].size(), out_file_path.c_str());
    }
  }
}
