...
Status ShlExecutionProvider::Compile(const std::vector<FusedNodeAndGraph>& fused_nodes_and_graphs,
  ...
  for (const auto& fused_node_graph : fused_nodes_and_graphs) {
    ...
    std::vector<void*> input_data_buffers;
    ...
    // make sure current session has dynamic input or not
    bool is_dynamic_shape = false;
    std::map<int, struct csinn_tensor*> need_malloc_tensors;
    const auto& input_tensors = graph_view.GetInputs();
    for (int i = 0; i < input_num; i++) {
      csinn_tensor* shl_input = csinn_alloc_tensor(current_sess);
      csinn_get_input(i, shl_input, current_sess);
      auto in_data_dtype = shl_ep::GetShlDtypeEnum(input_tensors[i]->TypeAsProto()->tensor_type());
      ...
      if (shl_input->dtype != in_data_dtype) {
        need_malloc_tensors[i] = shl_input;
      } else {
        need_malloc_tensors[i] = nullptr;
      }
    }

    // second, prepare all input data buffer
    for (const auto& pair : need_malloc_tensors) {
      if (pair.second && !is_dynamic_shape) {
        input_data_buffers.push_back(shl_mem_alloc_aligned(csinn_tensor_byte_size(pair.second), 0));
      } else {
        input_data_buffers.push_back(nullptr);
      }
    }
    ...
    compute_info.compute_func = [=](FunctionState state,
      ...
      for (int i = 0; i < input_num; i++) {
        ...
        if (shl_input->dtype != in_data_dtype) {
          if (current_sess->dynamic_shape) {
            ...
            shl_input->data = input_data_buffers[i];
          }
#if defined(SIM_ON_X86) || defined(RISCV_C906)
...
#else
          int size = csinn_tensor_size(shl_input);
          float scale = shl_input->qinfo->scale;
          int32_t zero_point = shl_input->qinfo->zero_point;
          if (in_data_dtype == CSINN_DTYPE_FLOAT32) {
            auto input_data = static_cast<const float*>(input_tensor.GetTensorRawData());
            auto shl_data = static_cast<uint8_t*>(shl_input->data);
            shl_c920_f32_to_u8(input_data, shl_data, zero_point, &scale, size);
          } else {
            throw std::invalid_argument("The input dtype not support");
          }
      ...
      if (config_.count("debug_run_time")) {
      ...
      for (size_t i = 0; i < n_outputs; i++) {
        ...
        float* output_buf = reinterpret_cast<float*>(const_cast<void*>(output_tensor.GetTensorRawData()));
...
#else
          float scale = shl_output->qinfo->scale;
          int32_t zero_point = shl_output->qinfo->zero_point;
          int out_size = csinn_tensor_size(shl_output);
          if (shl_output->dtype == CSINN_DTYPE_UINT8) {
            shl_c920_u8_to_f32(static_cast<uint8_t*>(shl_output->data), output_buf, zero_point, &scale, out_size);
          } else {
            throw std::invalid_argument("The output dtype not support");
          }
#endif
        } else {
