...
csinn_layout_enum GetShlActLayoutEnum(int dim_count) {
  ...
  throw std::invalid_argument("Get unsupport layout.");
  return CSINN_LAYOUT_NCHW;
}
...
std::pair<bool, std::string> IsNodeSupported(const GraphViewer& graph_viewer, const Node* node, csinn_api_enum base_api) {
  ...
  const std::vector<std::string> cpu_supported_ops{
      ...
      "MatMul", "LeakyRelu", "Softmax",
      "Erf", "DequantizeLinear", "QuantizeLinear", "Resize"
      // "Sqrt", "Div", "Sub", "Pow", "ReduceMean", "LayerNormalization",
      // "Sub", "Pow", "Resize", "Unsqueeze",
  ...
  const std::vector<std::string> th1520_supported_ops{
      "Add", "AveragePool", "Clip", "Concat",
      "Conv", "Flatten", "Gemm", "GlobalAveragePool",
      "MaxPool", "Mul", "Relu", "Sigmoid", "Softmax",
      "Split", "Transpose", "Reshape", "Resize",
      "LeakyRelu", "DequantizeLinear", "QuantizeLinear"};
  ...
  if (op == "Conv") {
    ...
    if (in_shape && !is_one_of<int64_t>(in_shape->dim_size(), 4)) {
    ...
    auto shape_tensor_proto = graph_viewer.GetConstantInitializer(shape->Name(), true);
    if (shape_tensor_proto) {
      Initializer unpacked_tensor(*shape_tensor_proto);
      const uint8_t* data_buf = unpacked_tensor.DataAsByteSpan().data();
      const int64_t* int64_data = reinterpret_cast<const int64_t*>(data_buf);
      int neg_shape_count = 0;
      for (int i = 0; i < shape_tensor_proto->dims()[0]; i++) {
        if (int64_data[i] <= 0) {
          neg_shape_count++;
        }
        if (neg_shape_count > 1) {
          return {false, "Reshape only supported constant shape"};
        }
      }
    }
